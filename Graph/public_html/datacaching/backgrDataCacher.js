...
startBackgroundCaching = function()
{
    ...
    socket.openSocket();
    socket.setOnOpenCallback(onReadyOpeningSocket);
    socket.setOnMessageCallback(onMessageRecieved);
}
;

function onReadyOpeningSocket()
{
...
function onReturnResult(req, results)
{
    if (results.rows.length !== 0)
    {
        ...
        if (returnedBeginTime > beginTime && returnedEndTime == endTime)
        {
            ...
            socket.sendMessage(tableName + ';' + needenTime + ';' + '1' + ';' + aggregation + ';' + channelCount + ';' + db_items + ';');
        }
        if (returnedBeginTime == beginTime && returnedEndTime < endTime)
        {
        ...
        if (beginTime < returnedBeginTime && endTime > returnedEndTime)
        {
            ...
            var needenTime1 = beginTime + '-' + endTime;
            socket.sendMessage(tableName + ';' + needenTime1 + ';' + '1' + ';' + aggregation + ';' + channelCount + ';' + db_items + ';');
        }
    }
    else
    {
